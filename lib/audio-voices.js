...
export function createBassVoice(audioContext, midi, bassGainNode, time, layers, glide = 0.04, filter = 420, release = 0.22, glideFromMidi = null, velocity = 1, releaseAt = null, harmonics = 0, harmonicShape = "square", harmonicFilter = 300) {
  ...
  const harmonicVoices = [];
  if (harmonics > 0) {
    const frequency = midiToHz(midi);
    const harmonicOsc = audioContext.createOscillator();
    const harmonicEnv = audioContext.createGain();
    const harmonicFilterNode = audioContext.createBiquadFilter();
    
    harmonicOsc.type = harmonicShape;
    harmonicOsc.frequency.setValueAtTime(frequency, time);
    
    harmonicFilterNode.type = "bandpass";
    harmonicFilterNode.frequency.setValueAtTime(harmonicFilter, time);
    harmonicFilterNode.Q.setValueAtTime(1, time);
    
    const harmonicGain = harmonics * 0.15;
    harmonicEnv.gain.setValueAtTime(0.0001, time);
    harmonicEnv.gain.exponentialRampToValueAtTime(Math.max(0.0001, harmonicGain), time + 0.015);
    
    harmonicOsc.connect(harmonicFilterNode).connect(harmonicEnv).connect(destination);
    harmonicOsc.start(time);
    if (releaseAt !== null) {
      harmonicEnv.gain.setTargetAtTime(0.0001, releaseAt, release);
      harmonicOsc.stop(releaseAt + release + 0.1);
    }
    harmonicVoices.push({ osc: harmonicOsc, envelope: harmonicEnv });
  }

  return { destination, filter: filterNode, voices, harmonicVoices };
}
...
export function releaseBassVoice(voices, time, release = 0.22) {
  ...
  if (voices.harmonicVoices) {
    voices.harmonicVoices.forEach((voice) => {
      voice.envelope.gain.cancelScheduledValues(time);
      voice.envelope.gain.setTargetAtTime(0.0001, time, release);
      voice.osc.stop(time + release + 0.1);
    });

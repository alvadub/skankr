...
function parseTempoMetadata(raw) {
  ...
  tokens.forEach((token) => {
    const tempoMatch = token.match(/^(\d+)$/);
    if (tempoMatch) {
      const tempo = parseInt(tempoMatch[1], 10);
      if (!Number.isFinite(tempo) || tempo <= 0) {
        throw new Error(`Invalid tempo '${token}'`);
      }
      meta.tempo = tempo;
      return;
    }
    ...
    if (meterMatch) {
      const numerator = parseInt(meterMatch[1], 10);
      const denominator = parseInt(meterMatch[2], 10);
      if (!Number.isFinite(numerator) || numerator <= 0) {
        throw new Error(`Invalid meter numerator '${meterMatch[1]}'`);
      }
      if (![2, 4, 8, 16].includes(denominator)) {
        throw new Error(`Invalid meter denominator '${meterMatch[2]}'. Use one of 2, 4, 8, 16`);
      }
    ...
    const subdivMatch = token.match(/^(\d+)(n|t)$/);
    if (subdivMatch) {
      const validSubdivs = ["1n", "2n", "4n", "4t", "8n", "8t", "16n", "16t", "32n"];
      if (!validSubdivs.includes(token)) {
        throw new Error(`Invalid subdivision '${token}'. Use one of ${validSubdivs.join(", ")}`);
      }
      meta.subdiv = token;
      return;
    }

    throw new Error(`Invalid tempo metadata token '${token}'`);
  });

  if (!meta.tempo && !meterFound && !meta.subdiv) {
    throw new Error(`Invalid tempo metadata '${raw}'. At least one of tempo, meter, or subdivision is required`);
  }
...
function normalizeMetadataToken(token) {
  if (token && typeof token.value === "string" && /^[a-z]$/.test(token.value)) {
  ...
  buffer.split(/\r?\n/g).forEach((rawLine, nth) => {
    ...
    try {
      ...
      if (line.charAt() === "%") {
        ...
        data[key] = tokens.map(normalizeMetadataToken);
      } else {

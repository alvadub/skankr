...
import { SUBDIVISIONS, subdivisionToBeats, isValidSubdivision } from "../skt.js";
...
export class AudioRuntime {
  constructor(ctx, resolver, trackKeys = ["kick", "snare", "hihat", "openhat"], options = {}) {
    ...
    this.options = {
      ...
      subdiv: isValidSubdivision(options.subdiv) ? options.subdiv : "16n",
    };
    ...
    this.subdiv = this.options.subdiv;
  ...
  playStep(stepIndex, time) {
    ...
    if (stepEvent.drums && stepEvent.drums.length > 0) {
      const subdivBeats = subdivisionToBeats(this.subdiv);
      const stepDuration = (60 / this.bpm) * subdivBeats;

...
import { parseChord as parseChordFn, parseNoteName } from "./audio-math.js";
import { SUBDIVISIONS, DEFAULT_SUBDIVISIONS, subdivisionToBeats, isValidSubdivision } from "../skt.js";
import { DRUM_STEPS, fixedLengthArray, normalizeDrumValue, drumLengthArray } from "../codec.js";
...
export function validateDrumPattern(value) {
  const raw = String(value || "").trim();
  if (!raw) return { valid: false, error: "Empty pattern" };
  if (/[^xX_\-\[\]\s|.0]/.test(raw)) {
    const invalid = raw.match(/[^xX_\-\[\]\s|.0]/g);
    return { valid: false, error: `Invalid character: ${[...new Set(invalid)].join(", ")}` };
  }
  let steps = 0;
  for (let i = 0; i < raw.length; i += 1) {
    const ch = raw[i];
    if (ch === "[") {
      const end = raw.indexOf("]", i + 1);
      if (end < 0) return { valid: false, error: "Unclosed bracket" };
      steps += 1;
      i = end;
      continue;
    }
  ...
  if (steps === 0) return { valid: false, error: "Empty pattern" };
  if (steps > 32) return { valid: false, error: `${steps} steps (max 32)` };
  if (32 % steps !== 0) return { valid: false, error: `${steps} steps (must divide evenly into 32: 1, 2, 4, 8, 16, or 32)` };
  return { valid: true, error: null };
}

export function validateChordPattern(value, maxSteps = 32) {
  const raw = String(value || "").trim();
  if (!raw) return { valid: true, error: null, stats: { pulses: 0, sustains: 0, rests: 0, steps: 0 } };
  if (/[^xX_\-\[\]\s|.0]/.test(raw)) {
    const invalid = raw.match(/[^xX_\-\[\]\s|.0]/g);
    return { valid: false, error: `Invalid character: ${[...new Set(invalid)].join(", ")}` };
  }
  const symbols = splitPatternWithSubsteps(raw.replace(/[\s|]/g, "").replace(/[.0]/g, "-"));
  const flat = symbols.flat(1);
  if (!flat.length) return { valid: true, error: null, stats: { pulses: 0, sustains: 0, rests: 0, steps: 0 } };
  if (flat.length > maxSteps) return { valid: false, error: `${flat.length} steps (max ${maxSteps})` };
  const stats = chordPatternStats(symbols);
  return { valid: true, error: null, stats };
}

export function validateBassPattern(value, maxTicks = 128) {
  const raw = String(value || "").trim();
  if (!raw) return { valid: true, error: null, stats: { pulses: 0, sustains: 0, rests: 0, ticks: 0 } };
  if (/[^xX_\-\[\]\s|.0]/.test(raw)) {
    const invalid = raw.match(/[^xX_\-\[\]\s|.0]/g);
    return { valid: false, error: `Invalid character: ${[...new Set(invalid)].join(", ")}` };
  }
  const symbols = splitPatternWithSubsteps(raw.replace(/[\s|]/g, "").replace(/[.0]/g, "-"));
  const flat = symbols.flat(1);
  if (!flat.length) return { valid: true, error: null, stats: { pulses: 0, sustains: 0, rests: 0, ticks: 0 } };
  if (flat.length > maxTicks) return { valid: false, error: `${flat.length} ticks (max ${maxTicks})` };
  let pulses = 0;
  let sustains = 0;
  let rests = 0;
  ...
  return { valid: true, error: null, stats: { pulses, sustains, rests, ticks: flat.length } };
}

export function validateChordPool(value) {
  const raw = String(value || "").trim();
  if (!raw) return { valid: true, error: null, chords: [] };
  const tokens = splitWhitespacePreservingParts(raw).filter((part) => !/^\s+$/.test(part));
  if (!tokens.length) return { valid: true, error: null, chords: [] };
  const invalidTokens = [];
  const chords = tokens.map((token) => {
    const chord = parseChordFn(token);
    if (!chord) invalidTokens.push(token);
    return chord ? chord.label : null;
  });
  if (invalidTokens.length) {
    return { valid: false, error: `Invalid chord: ${[...new Set(invalidTokens)].join(", ")}` };
  }
  return { valid: true, error: null, chords };
}

export function validateBassNotes(value, expectedCount = 0) {
  const raw = String(value || "").trim();
  if (!raw) return { valid: true, error: null, notes: [], count: 0 };
  const notes = parseBassNotesFn(raw);
  if (!notes) return { valid: false, error: "Invalid note format" };
  if (expectedCount > 0 && notes.length !== expectedCount) {
    return { valid: false, error: `${notes.length} notes (expected ${expectedCount})`, notes, count: notes.length };
  }
  return { valid: true, error: null, notes, count: notes.length };
}
...
export function drumValuesToHits(values) {
...
export function drumHitsToValues(hits) {
  return fixedLengthArray(hits, null, DRUM_STEPS).map((step) => {
    if (!step || step.length === 0) return 0;
    return step[0]?.vel || 0;
  });
}

export function parsePatternWithSubdiv(rawPattern, subdiv = "4n", maxSteps = 32) {
  const raw = String(rawPattern || "").trim();
  if (!raw) return null;
  if (/[^xX_\-\[\]\s|.0]/.test(raw)) return null;
  
  const subdivBeats = subdivisionToBeats(subdiv);
  const steps = [];
  let charIndex = 0;
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    if (ch === "[") {
      ...
      const subChars = raw.slice(i + 1, end).split("");
      if (!subChars.length) return null;
      const subhits = [];
      subChars.forEach((c, idx) => {
        const pos = idx / subChars.length;
        if (c === "x") subhits.push({ pos, vel: 0.72 });
        else if (c === "X") subhits.push({ pos, vel: 1 });
        else if (c === "_") subhits.push({ pos, vel: 0.72, sustain: true });
      });
      steps.push(subhits.length > 0 ? { type: "subhits", hits: subhits } : { type: "rest" });
      i = end;
      continue;
    }
    if (/[\s|]/.test(ch)) continue;
    const symbol = ch === "." || ch === "0" ? "-" : ch;
    if (!["x", "X", "_", "-"].includes(symbol)) return null;
    if (symbol === "x") steps.push({ type: "hit", velocity: 0.72 });
    else if (symbol === "X") steps.push({ type: "hit", velocity: 1 });
    else if (symbol === "_") steps.push({ type: "sustain" });
    else steps.push({ type: "rest" });
  }
  
  if (!steps.length || steps.length > maxSteps) return null;
  
  return {
    steps,
    subdiv,
    subdivBeats,
    stepDurationBeats: subdivBeats,
  };
}

export function patternStepToHits(step) {
  if (!step || step.type === "rest") return null;
  if (step.type === "hit") {
    return [{ pos: 0, vel: step.velocity }];
  }
  if (step.type === "sustain") {
    return [{ pos: 0, vel: 0.72, sustain: true }];
  }
  if (step.type === "subhits") {
    return step.hits;
  }
  return null;
}
...
export const SUBDIVISION_OPTIONS = [
  ...
  { value: "4t", label: "Q. Triplet" },
  { value: "8n", label: "Eighth" },
  { value: "8t", label: "E. Triplet" },
  { value: "16n", label: "Sixteenth" },
  { value: "16t", label: "S. Triplet" },
  { value: "32n", label: "32nd" },
];

export function createSubdivisionSelect(currentValue = "4n", laneType = "drums") {
  const select = document.createElement("select");
  select.className = "subdivision-select";
  select.dataset.laneType = laneType;
  SUBDIVISION_OPTIONS.forEach((opt) => {
...
export function formatMeterDisplay(meter) {
  if (!meter || !Array.isArray(meter) || meter.length !== 2) return "4/4";
  return `${meter[0]}/${meter[1]}`;
}

export function formatSubdivisionDisplay(subdiv) {
  if (!subdiv) return "";
  const opt = SUBDIVISION_OPTIONS.find((o) => o.value === subdiv);
  return opt ? opt.label : subdiv;
}

export function renderBassNotesPreview(preview, rawNotes, rawPattern, expectedNotes = 0, activeNoteIndex = -1) {
  if (!preview) return;
  ...
  if (needsRebuild) {
    ...
    parts.forEach((part) => {
      ...
      const parsed = parseNoteName(part);
      if (!parsed) {
...
export function renderBassPatternPreview(preview, rawPattern, activeTick = -1, maxTicks = 128) {
  if (!preview) return;
  ...
  const patternLength = preview.querySelectorAll("[data-tick]").length;
  const wrappedTick = patternLength > 0 ? activeTick % patternLength : activeTick;
  preview.querySelectorAll("[data-tick]").forEach((cell) => {
    const tick = Number(cell.dataset.tick);
    cell.classList.toggle("playing", tick === wrappedTick);
  });
}
...
export function renderDrumPatternPreview(preview, rawPattern, activeStep = -1, maxSteps = 32) {
  ...
  const patternLength = preview.querySelectorAll("[data-step]").length;
  const wrappedStep = patternLength > 0 ? activeStep % patternLength : activeStep;
  preview.querySelectorAll("[data-step]").forEach((cell) => {
    const step = Number(cell.dataset.step);
    cell.classList.toggle("playing", step === wrappedStep);
  });
}
...
export function renderChordPatternPreview(preview, rawPattern, stepOffset = 0, maxSteps = 8) {
  ...
  preview.querySelectorAll("[data-step]").forEach((cell) => {
    const step = Number(cell.dataset.step);
    cell.classList.toggle("playing", wrappedStep === step);
  });
}
...
export function summarizeDrumTrack(values, stepCount = 32) {
  const normalized = values.map((v) => {
    ...
    if (Array.isArray(v) && v.length > 0) {
      const maxVel = Math.max(...v.map((h) => h?.vel || 0));
      if (maxVel >= 0.95) return 1;
      if (maxVel > 0) return 0.72;
      return 0;
    }
    return 0;
  });
...
export function createBlankScene(index, tracks, chordSteps = 32, chordEditorParts = 2, drumSteps = 32) {
  ...
  return {
    ...
    drums: Object.fromEntries(trackList.map((track) => [track.key, Array(drumSteps).fill(null)])),
    mutes: {
      rhythm: false,
      harmony: false,
      bass: false,
